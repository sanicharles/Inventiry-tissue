...
export const Layout: React.FC<LayoutProps> = ({ children, activeTab, onTabChange, selectedDate, onDateChange, records }) => {
  ...
  const renderCalendarOverlay = () => {
    ...
    const startPadding = new Date(year, month, 1).getDay(); // 0 = Sunday
      ...
      <>
        ...
        <div className="fixed top-20 left-4 right-4 bg-white rounded-[32px] p-6 shadow-2xl z-50 animate-slideUp border border-gray-100 max-w-md mx-auto">
          ...
          <div className="grid grid-cols-7 gap-2">
            {dayLabels.map(l => (
              <div key={l} className="text-[10px] text-center font-black text-gray-300 py-1">{l}</div>
            ))}
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  className={`relative aspect-square rounded-2xl flex items-center justify-center text-sm font-bold transition-all ${
                    isSelected 
                      ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' 
                      : 'bg-gray-50 text-gray-600 active:bg-blue-50'
                  }`}
                >
                  {day}
                  {dayHasData(dateObj) && !isSelected && (
          ...
          <button 
            ...
            className="w-full mt-6 py-3 bg-gray-50 text-gray-500 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-gray-100"
          >
            Kembali ke Hari Ini
          </button>
        </div>
    ...
    <div className="flex flex-col h-screen max-w-md mx-auto bg-gray-50 overflow-hidden shadow-xl border-x border-gray-200">
      <header className="bg-white/80 backdrop-blur-md px-4 py-4 border-b border-gray-100 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-black text-gray-800 tracking-tight">TissueRoll</h1>
        </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-md border-t border-gray-200 flex justify-around items-center py-3 px-6 z-20">
        <button onClick={() => onTabChange('entry')} className={`flex flex-col items-center gap-1 transition-colors ${activeTab === 'entry' ? 'text-blue-600' : 'text-gray-400'}`}>
        ...
        <button onClick={() => onTabChange('scan')} className={`flex flex-col items-center gap-1 -mt-8 bg-blue-600 w-14 h-14 rounded-full shadow-lg border-4 border-white justify-center transition-all active:scale-90 ${activeTab === 'scan' ? 'bg-blue-700' : ''}`}>
        ...
        <button onClick={() => onTabChange('reports')} className={`flex flex-col items-center gap-1 transition-colors ${activeTab === 'reports' ? 'text-blue-600' : 'text-gray-400'}`}>
